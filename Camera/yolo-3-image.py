...
start = time.time()
#This method performs a forward pass through the network, meaning it processes the input blob through all the layers of the neural network.
output_from_network = network.forward(layers_names_output)
end = time.time()

# Showing spent time for forward pass
#{:.5f}'.format(...): This formats the time difference to five decimal places.
# print('Objects Detection took {:.5f} seconds'.format(end - start))
# print('output network', output_from_network)
...
# This line starts a loop to iterate through the output from the YOLO network. The output_from_network likely contains the results of object detection for a single frame or image.
for result in output_from_network:
    # Going through all detections from current output layer
    # Within the outer loop, this line iterates through the detected objects within the current output. Each detected_objects variable likely represents one detected object in the image.
    for detected_objects in result:
        # Getting 80 classes' probabilities for current detected object
        # Each bounding box has (5 + c ) attributes include:  Centre coordinate of bounding box, Width and height that are dimensions of bounding box, Objectness score, List of confdences for every class this bounding box might belong to
        scores = detected_objects[5:]
        # Getting index of the class with the maximum value of probability
        # Here, the code finds the index of the class with the maximum probability from the scores array. This determines which class the current detected object most likely belongs to.
        class_current = np.argmax(scores)
        # The code extracts the confidence score (probability) of the detected object belonging to the class determined by class_current
